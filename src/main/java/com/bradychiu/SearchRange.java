...
public class SearchRange {
    ...
    public static int[] searchRangeBinaryRecursive(int[] nums, int target) {
        if (nums == null || nums.length == 0)
            return new int[]{-1, 1};
        int lo = firstGreaterEqualBinaryRecursive(nums, target, 0, nums.length - 1);
        if (lo == nums.length || nums[lo] != target)
            return new int[]{-1, 1};
        int nextTargetLo = firstGreaterEqualBinaryRecursive(nums, target + 1, lo + 1, nums.length - 1);
    ...
    private static int firstGreaterEqualBinaryRecursive(int[] nums, int target, int lo, int hi) {
        ...
        if (nums[mid] < target)
            ...
            return firstGreaterEqualBinaryRecursive(nums, target, lo, mid - 1);
